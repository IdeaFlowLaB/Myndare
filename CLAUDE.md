...
## 同步 LicenseKing `/r/` 公開頁（從 Licenseking_ai 來的 `拷貝` 檔）

Mac 端 CC 在 Licenseking_ai repo 維護履歷卡公開頁的 source（[licenseking/r/index.html](licenseking/r/index.html) / [render.js](licenseking/r/render.js) / [style.css](licenseking/r/style.css)），會以 `index拷貝.html` / `render拷貝.js` / `style拷貝.css` 後綴丟進 myndare 的 [licenseking/r/](licenseking/r/) 等同步。Mac 端不一定知道 myndare 這邊做過的修正，**直接覆蓋會回歸出 bug**。

**已知踩過的雷（每次同步都要逐項檢查）**：

1. **`render.js`：本地變數絕對不能叫 `supabase`**
   - 必須是 `const sb = window.supabase.createClient(...)`
   - 原因：Supabase UMD CDN 已把 `supabase` 註冊為全域 var，用 `const supabase = ...` 會撞名拋 `Identifier 'supabase' has already been declared`，整個 script parse 失敗、`loadPage()` 不執行、spinner 永遠轉、三個錯誤狀態都看不到
   - `loadPage()` 內呼叫處也要用 `await sb.rpc(...)`

2. **`index.html`：必須保留以下 head 元素**（Mac 端常退化掉）
   - `<html lang="zh-Hant-TW">`（不是 `zh-Hant`）
   - `<meta name="robots" content="noindex, follow">`（裸網址是錯誤狀態，不能被 Google 收錄）
   - `<meta property="og:locale" content="zh_TW">`
   - `<meta property="og:url" content="https://myndare.com/licenseking/r/">`
   - `<meta property="og:image" content="https://myndare.com/licenseking/social-banner.png">`（**不是** `og-default.png`，那檔不存在）
   - `og:image:width/height/alt` 三個
   - favicon 兩條（絕對路徑 `/favicon.ico`、`/favicon.png`）
   - Google Fonts 用 `media="print" onload="this.media='all'"` + `<noscript>` fallback（避免 render-blocking）
   - twitter:image 也是 `social-banner.png`

3. **Cache-bust 版本號**（GitHub Pages 對 `render.js` / `style.css` 回 `Cache-Control: max-age=600`，邊緣節點 + 瀏覽器會吃舊版）
   - `<script src="render.js?v=x.x.x">` 跟 `<link href="style.css?v=x.x.x">` 兩處都要 bump

### Workflow

1. **`ls licenseking/r/`** 看有哪些 `拷貝` 檔
2. **逐檔 Read** `index拷貝.html` / `render拷貝.js` / `style拷貝.css`，跟現有檔 diff
3. **逐檔分類**：
   - 有回歸（上面任何一項雷）→ **不覆蓋**現有檔，只把現有檔的 cache-bust 版本號 bump 對齊
   - 完全相同 → 不動
   - 無回歸 + 有實質新功能 → 覆蓋
4. **`rm` 3 個 `拷貝` 檔**
5. **`node scripts/check-seo.js`** 必須 22/22 通過、`[utility] licenseking/r/index.html` 在裡面
6. **本機 Playwright 驗證**（這步絕對不能省，之前線上才發現 spinner bug）：
   ```powershell
   # 啟動本機 server（background）
   python -m http.server 8765 --bind 127.0.0.1

   # Playwright 開三個 URL 確認：
   # http://127.0.0.1:8765/licenseking/r/             → "連結格式錯誤"
   # http://127.0.0.1:8765/licenseking/r/?id=invalid  → "連結格式錯誤"（7 字元）
   # http://127.0.0.1:8765/licenseking/r/?id=zzzzzzzz → "此連結不存在或已停用"（8 字元，走真實 Supabase RPC）

   # 同時驗證 console errors = 0
   ```
7. **commit + push**（pre-commit hook 會再跑一次 check-seo.js）
8. **告知 Mac 端**：如果發現 render.js 或 index.html 有回歸 bug，提醒 Wade 把修正同步回 Licenseking_ai 的 source，避免下次又踩同樣的雷
